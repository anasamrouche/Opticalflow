...
mod horn_schunck_rs {
        ...
        for _ in 0..max_iter {
            for x in 0..image_height {
                for y in 0..image_width {
                    ...
                    u_field[[x, y]] = u_average
                        - x_derivative[[x, y]]
                            * (x_derivative[[x, y]] * u_average
                                + y_derivative[[x, y]] * v_average
                                + time_derivative[[x, y]])
                            / (alpha_squared
                                + x_derivative[[x, y]].powi(2)
                                + y_derivative[[x, y]].powi(2));
                    v_field[[x, y]] = v_average
                        - y_derivative[[x, y]]
                            * (x_derivative[[x, y]] * u_average
                                + y_derivative[[x, y]] * v_average
                                + time_derivative[[x, y]])
    ...
    fn solve_gauss_seidel<'py>(
        ...
        let (frame_count, frame_height, frame_width) = (
            video_array.shape()[0],
            video_array.shape()[1],
            video_array.shape()[2],
        );
    ...
    fn gradient_descent(
        ...
        if norm_l2 {
            ...
            let get_cross_pattern = |field: &Array2<f64>, x_index: usize, y_index: usize| -> f64 {
                ...
                (field[[x_next, y_index]] - field[[x_index, y_index]]).signum()
                    + (field[[x_index, y_index]] - field[[x_previous, y_index]]).signum()
                    + (field[[x_index, y_next]] - field[[x_index, y_index]]).signum()
                    + (field[[x_index, y_index]] - field[[x_index, y_previous]]).signum()
            };
    ...
    fn gauss_seidel_from_previous_uv(
        image1: ArrayView2<'_, f64>,
        image2: ArrayView2<'_, f64>,
        prev_u_field: Array2<f64>,
        prev_v_field: Array2<f64>,
        alpha_squared: f64,
        iter_max: u32,
    ) -> Fields {
